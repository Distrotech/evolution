...
mail_composer_prefs_construct (MailComposerPrefs *prefs)
{
	...
	gui = glade_xml_new (EVOLUTION_GLADEDIR "/mail-config.glade", "composer_prefs_tab");
	prefs->gui = gui;
...
#define CONFIG_CONTROL_FACTORY_ID "OAFIID:GNOME_Evolution_Mail_ComposerPrefs_ConfigControlFactory"
...
{
	MailComposerPrefs *prefs = (MailComposerPrefs *) data;
...
config_control_apply_callback (EvolutionConfigControl *config_control, void *data)
{
	...
	mail_composer_prefs_apply (prefs);
}
...
gboolean
mail_composer_prefs_register_factory (GNOME_Evolution_Shell shell)
{

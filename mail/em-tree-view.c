...
emtv_cell_date(GtkTreeViewColumn *col, GtkCellRenderer *cell, GtkTreeModel *model, GtkTreeIter *iter, void *data)
{
	...
	if (mi) {
		guint32 flags = camel_message_info_flags(mi);
		time_t date = camel_message_info_date_received(mi);
...
emtv_cell_properties(GtkTreeViewColumn *col, GtkCellRenderer *cell, GtkTreeModel *model, GtkTreeIter *iter, void *data)
{
	...
	if (mi) {
		...
		colour = camel_message_info_user_tag(mi, "colour");
		if (colour == NULL) {
			const char *label;

			if ((label = camel_message_info_user_tag(mi, "label")) == NULL
				...
				if (flags & CAMEL_MESSAGE_FLAGGED) {
					...
					due = camel_message_info_user_tag(mi, "due-by");

...
mail_vfolder_add_uri(CamelStore *store, const char *uri, int remove)
{
	...
	if (CAMEL_IS_VEE_STORE(store) || !strncmp(uri, "vtrash:", 7) || !strncmp(uri, "vspam:", 6) || context == NULL)
		return;
...
mail_vfolder_delete_uri(CamelStore *store, const char *uri)
{
	...
	if (context == NULL || !strncmp(uri, "vtrash:", 7) || !strncmp(uri, "vspam:", 6))
		return;
...
mail_vfolder_rename_uri(CamelStore *store, const char *from, const char *to)
{
	...
	if (context == NULL || !strncmp(from, "vtrash:", 7) || !strncmp(to, "vtrash:", 7) || !strncmp(from, "vspam:", 6) || !strncmp(to, "vspam:", 6))
		return;
